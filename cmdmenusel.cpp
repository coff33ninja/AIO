...
int main(int argc, char *argv[])
{
    if (argc <= 2)
    {
        cout << "CmdMenuSel (V1.4)" << endl
             ...
             << "option was specified in." << endl
             << endl
             << "USAGE:" << endl
             << endl
             << "CmdMenuSel_2 FBFB {\"Option 1\"} [\"Option 2\"] [....] " << endl
             << endl
             << endl
             << "All option string are trimmed to the console width. The number of specified" << endl
             << "options must be less than or equal to the number of lines in the console." << endl
             << endl
             ...
             << "See \"COLOR /?\" for colo[u]r code listings." << endl
             << endl
             ...
             << "  Selected and unselected background colo[u]rs are the same." << endl
             << endl

             << "If an error occurs the return code will be zero and a string will be set" << endl
             << "to the console error stream." << endl
             << endl
    ...
    if (retcode != 0)
    {
        ...
        sChopper = argv[1];
        if (
            sChopper.length() == 4 && sChopper.substr(0, 1) != sChopper.substr(1, 1) && sChopper.substr(2, 1) != sChopper.substr(3, 1) && sChopper.substr(1, 1) != sChopper.substr(3, 1))
        {
        ...
        if (coMenuPos.Y + wMenuItems - 1 > sbiConsoleInfo.srWindow.Bottom && coMenuPos.Y != sbiConsoleInfo.srWindow.Top)
        {
            wRectYArea = sbiConsoleInfo.srWindow.Bottom - sbiConsoleInfo.srWindow.Top;
    ...
    while (retcode == 10000)
    {
        ...
        if (irUserInput.EventType == MOUSE_EVENT)
        {
            ...
            if (irUserInput.Event.MouseEvent.dwButtonState & FROM_LEFT_1ST_BUTTON_PRESSED)
            {
                ...
                if (irUserInput.Event.MouseEvent.dwMousePosition.Y >= coMenuPos.Y && irUserInput.Event.MouseEvent.dwMousePosition.Y < coMenuPos.Y + wMenuItems)
                {
            ...
            else
            {
                if (irUserInput.Event.MouseEvent.dwButtonState == 0)
                {
                    ...
                    if (bMouseDown)
                    {
                        if (irUserInput.Event.MouseEvent.dwMousePosition.Y - coMenuPos.Y == wMouseDownPos)
                        {
        ...
        if (irUserInput.EventType == KEY_EVENT)
        {
            ...
            if (irUserInput.Event.KeyEvent.wVirtualKeyCode == VK_SPACE || irUserInput.Event.KeyEvent.wVirtualKeyCode == VK_RETURN)
            {
                bKeyDown = true;
                continue;
            }
