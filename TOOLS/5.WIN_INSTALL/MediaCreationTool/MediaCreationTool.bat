...
:: Changelog: 2022.03.18
:: - prevent more launch errors; fix Auto Upgrade regression; USBLayout progress; pickup $ISO$ dir to add on media
...
echo;20220318>latest & if %latest% lss 20211116 del /f /q products*.* MediaCreationTool*.exe >nul 2>nul
...
set "0=%~f0" & start /wait /b powershell -nop -c "iex ([io.file]::ReadAllText($env:0) -split '[:]Assisted_MCT')[1];"
...
#:: unreliable processing like pausing setuphost removed; enhanced output; automation fixes; iso output path kung-fu
 ...
 $ESD = $null; $USB = $null; $ISO = "$envOOT\$env:X $env:VIS $env:MEDIA_CFG $env:MEDIA_ARCH $env:MEDIA_LANGCODE.iso"
 ...
 :mct while ($true) {
  ...
  #:: watch usb layout progress if target is $USB
   if ($null -ne $USB) {
     ...
     while (-not $ready) {
       sleep -m 5000; if ($mct.HasExited) {write-host err; break :mct}
       [xml]$xml = get-content "$WS\Panther\windlp.state.xml"
       foreach ($t in $xml.WINDLP.TASK) { if ($t.Name -eq $task) { foreach ($a in $t.ACTION) { if ($a.ActionName -eq $action) {
        ...
        #:: hide setup ui and show script window in foreground (to prevent ui stalling / working dir cleanup when done)
         ...
         if ($foreground -eq 0 -and $done -gt $almost) {
           $set = get-process SetupHost -ea 0; $SetupHost = $set.MainWindowHandle; ShowWindow $SetupHost 0
           powershell -win 0 -nop -c ";"; ShowWindow (get-process -Id $PID).MainWindowHandle 1; $foreground = 1
         }
         write-host -nonew -fore Gray "`r${size} MB  written: ${done} MB "
         #$done = [Convert]::ToInt64($current, 16); if ($size -eq 0) {$size = [Convert]::ToInt64($total, 16); $80p = $size * 0.8}
       }}}}
     }
     write-host ok; write-host -fore Gray "Created", "UsbLayout"
   }
...
#:: skip windows 11 upgrade checks - for running setup from boot media
 if ($env:VER -ge 22000 -and (test-path "$DIR\sources\boot.wim")) {
   ...
   if (-not (test-path "$DIR\AutoUnattend.xml")) {
     $f4 = "$env:WORK\MCT\AutoUnattend.xml"; if (test-path $f4) {copy -path $f4 -dest "$WD\MOUNT" -force >$null}
   }
  ...
  #:: create media
   if ($null -ne $USB) {
     ...
     #:: if target is $ISO, load snippet then call MakeISO DIR ISO LABEL
  ...
  #:: cleanup
   ...
   cmd /d /x /c "rmdir /s /q ""$DIR"" >nul 2>nul & del /f /q ""$WS\*.*"" >nul 2>nul"
 }
...
@echo off& set title= Auto Upgrade with edition switch support
...
if "%Build%" geq "22000" if "%SKIP_11_SETUP_CHECKS%" equ "1" (set OPTIONS=%TRICK%%OPTIONS%)
